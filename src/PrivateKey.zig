...
test "parse ec pem" {
    ...
    try testing.expectEqualSlices(u8, &testu.hexToBytes(priv_key), k.secret_key);
...
test "parse rsa pem" {
    ...
    try testing.expectEqualSlices(u8, &testu.hexToBytes(modulus), k.modulus);
    try testing.expectEqualSlices(u8, &testu.hexToBytes(public_exponent), k.public_exponent);
    try testing.expectEqualSlices(u8, &testu.hexToBytes(private_exponent), k.private_exponent);
}
